# TEAM Task Manager Skill - v1.2.0 更新说明
...
### 1. 切换新 OpenAPI 应用
- AppKey 更新为新应用（appId: 556325）
- 默认项目从 P98273 切换为 P42952
- 默认操作人：liyuanmeng05
...
| 接口 | 说明 |
|------|------|
| `GET /project/authed` | 获取用户有权限的项目列表 |
| `GET /project/configInfo` | 查询项目的任务分类+任务组+优先级配置 |
| `GET /project/members` | 查询项目下的所有成员 |
| `GET /project/search` | 按关键词搜索项目 |

### 3. 新增任务组类接口（4个）

| 接口 | 说明 |
|------|------|
| `POST /task/createSection` | 新建任务组 |
| `POST /task/changeSection` | 修改任务组名称/排序 |
| `POST /task/removeSection` | 删除任务组 |
| `POST /task/moveToSection` | 移动任务到指定任务组 |

### 4. 新增任务修改/删除接口（2个）

| 接口 | 说明 |
|------|------|
| `POST /task/modifyValues` | 修改任务字段（优先级/状态/执行人等） |
| `POST /task/deleteTask` | 删除任务 |
...
|---|------|------|------|
| 1 | 认证 | `/token/get` | POST |
| 2 | 任务查询 | `/task/listTaskClasses` | GET |
| 3 | 任务查询 | `/task/getFields` | GET |
| 4 | 任务创建 | `/task/create` | POST |
| 5 | **项目** | `/project/authed` | GET |
| 6 | **项目** | `/project/configInfo` | GET |
| 7 | **项目** | `/project/members` | GET |
| 8 | **项目** | `/project/search` | GET |
| 9 | **任务组** | `/task/createSection` | POST |
| 10 | **任务组** | `/task/changeSection` | POST |
| 11 | **任务组** | `/task/removeSection` | POST |
| 12 | **任务组** | `/task/moveToSection` | POST |
| 13 | **任务** | `/task/modifyValues` | POST |
| 14 | **任务** | `/task/deleteTask` | POST |
