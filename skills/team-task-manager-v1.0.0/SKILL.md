...
**用户触发创建任务关键词后，必须立即执行以下两步，不得询问任何问题，不得输出任何说明文字：**

```
第一步：后台静默获取 accessToken（execute_command，不输出）
第二步：直接输出 A2UI 表单 JSON（唯一输出内容）
```

**⛔ 禁止行为**：
- 禁止在输出 JSON 之前询问任务标题、执行人等任何信息
- 禁止输出"请告诉我..."、"需要以下信息..."等引导文字
- 禁止先解释流程再输出 JSON
- 禁止调用 use_skill(a2ui)，直接按模板生成 JSON

**✅ 正确交互模式**：
```
用户：创建 TEAM 任务
AI：[立即输出 A2UI JSON，无任何其他文字]

用户：[填写后发回 JSON]
AI：[立即调用 API 创建，返回任务 ID]
```

### A2UI 表单 JSON 模板（固定输出）

当用户触发创建任务时，**直接输出以下 JSON**（从 `.env` 读取默认值填充 `updateDataModel`）：

```json
[
  {
    "createSurface": {
      "surfaceId": "team-task-create-form",
      ...
      "components": [
        {
          "id": "root",
          "component": "Column",
          "children": ["form-title", "title-field", "description-field", "assignee-field", "reporter-field", "priority-field", "end-date-field", "participant-field", "action-row"]
        },
        { "id": "form-title", "component": "Text", "text": "创建 TEAM 任务", "variant": "h2" },
        { "id": "title-field", "component": "TextField", "variant": "shortText", "label": "任务标题 *", "placeholder": "请输入任务标题", "value": { "path": "/title" } },
        { "id": "description-field", "component": "TextField", "variant": "longText", "label": "任务描述", "placeholder": "请输入任务描述（可选）", "value": { "path": "/description" } },
        { "id": "assignee-field", "component": "TextField", "variant": "shortText", "label": "执行人 *", "placeholder": "快手用户名", "value": { "path": "/assignee" } },
        { "id": "reporter-field", "component": "TextField", "variant": "shortText", "label": "报告人 *", "placeholder": "快手用户名", "value": { "path": "/reporter" } },
        {
          "id": "priority-field",
        ...
        { "id": "end-date-field", "component": "DateTimeInput", "enableDate": true, "label": "截止日期", "value": { "path": "/endAt" } },
        { "id": "participant-field", "component": "TextField", "variant": "shortText", "label": "参与人", "placeholder": "多个用户名以逗号分隔", "value": { "path": "/participant" } },
        {
          "id": "action-row", "component": "Row", "justify": "end",
          "children": ["cancel-btn", "submit-btn"]
        },
        { "id": "cancel-btn", "component": "Button", "variant": "default", "child": "cancel-btn-text", "action": { "event": { "name": "cancel_form" } } },
        { "id": "cancel-btn-text", "component": "Text", "text": "取消", "variant": "body" },
        { "id": "submit-btn", "component": "Button", "variant": "primary", "child": "submit-btn-text", "action": { "event": { "name": "submit_task_form" } } },
        { "id": "submit-btn-text", "component": "Text", "text": "创建任务", "variant": "body" }
      ]
    }
  },
  {
...
**字段类型映射**（TEAM 字段类型 → A2UI 组件）：

| TEAM fieldType | A2UI 组件 | 说明 |
|---------------|-----------|------|
| `SHORT_TEXT` | `TextField(variant: shortText)` | 单行文本 |
| `LONG_TEXT` | `TextField(variant: longText)` | 多行文本 |
| `USER_PICKER` | `TextField(variant: shortText)` | 输入用户名 |
| `DROP_DOWN_SINGLE` | `ChoicePicker(variant: mutuallyExclusive)` | 单选，options 来自 fieldItemModels |
| `DROP_DOWN_MULTI` | `ChoicePicker(variant: multipleSelection)` | 多选 |
| `DATE_TIME` | `DateTimeInput` | 日期时间选择 |
| `NUMBER` | `TextField(variant: number)` | 数字输入 |
